  ...
  <div>
    ...
    <nav aria-label="Page navigation example">
      <ul class="pagination">
        ...
        <li v-if="page < $store.state.played.numberOfPages" class="page-item">
          <a
            class="page-link"
            href="#"
            aria-label="Next"
...
export default {
  ...
  computed: {
    pageOptions() {
      ...
      if (this.$store.state.played.numberOfPages <= 5) {
        for (let i = 1; i <= this.$store.state.played.numberOfPages; i++) {
          options.push(i);
        }
      } else if (this.page <= 3) {
        for (let i = 1; i <= 5; i++) {
      ...
      } else if (this.page >= this.$store.state.played.numberOfPages - 2) {
        for (
          let i = this.$store.state.played.numberOfPages - 4;
          i <= this.$store.state.played.numberOfPages;
          i++
        ) {
          options.push(i);
        }
